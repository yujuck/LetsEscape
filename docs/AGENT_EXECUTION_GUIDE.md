...
- [x] `store`, `theme`, `crawl_job`, `crawl_result` 테이블(또는 동등 구조) 설계/반영
...
- [x] `GET /stores` 서울 필터 포함 동작
- [x] `GET /themes` 검색/필터 동작 (권역, 인원, 공포도, 플레이시간)
- [x] 목록 API 응답에 페이지에서 필요한 필드가 누락 없이 포함
- [ ] API 실패 시 에러 응답 형식 일관성 유지

### C. Web (사용자 기능)

- [x] `/stores` 페이지가 실제 API 데이터로 렌더링
- [x] `/themes` 페이지가 실제 API 데이터로 렌더링
- [ ] 필터 UI와 API 파라미터 연동
- [x] 로딩/빈 상태/오류 상태 UI 반영
- [ ] `/reviews` 후기/기록 CRUD 기본 동작
- [x] 모바일/데스크톱 반응형 깨짐 없음

### D. 운영/품질

- [ ] 잘못 수집된 매장/테마를 수동 수정할 최소 관리자 경로 확보
- [ ] 중복 데이터 병합 또는 비활성 처리 정책 구현
- [x] `pnpm --filter @lets-escape/web build` 성공
...
- [x] `themes/stores`를 샘플 배열에서 API 실데이터 연동으로 전환
